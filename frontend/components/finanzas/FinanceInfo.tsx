...
export interface FinanceInfoContent {
  eyebrow?: string;
  title: string;
  summary: string;
  details: string[];
...
export function FinanceInfoButton({
  info,
  onOpen,
  className = "",
}: {
  info: FinanceInfoContent;
  onOpen: (info: FinanceInfoContent) => void;
  className?: string;
}) {
  return (
    <button
...
export function FinanceInfoModal({
  info,
  onClose,
}: {
  info: FinanceInfoContent | null;
  onClose: () => void;
}) {
  if (!info) {
    return null;
...
export function FinanceFieldLabel({
  ...
  optional?: boolean;
  info?: FinanceInfoContent;
  onInfo?: (info: FinanceInfoContent) => void;
}) {
  return (
    <span className="mb-1 flex flex-wrap items-center gap-2 text-sm font-medium text-zinc-400">
      ...
      {info && onInfo ? <FinanceInfoButton info={info} onOpen={onInfo} /> : null}
    </span>
  );
}
